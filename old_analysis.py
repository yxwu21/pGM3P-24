...
class TrajectoryAnalysis:
    ...
    def get_trajectory_info_custom_range(
        self, traj_folders, md_folder_path, traj_range=(-11, -1)
    ):
        trajectory = []
    ...
    def traj_analysis(
        self, md_path: Path, analysis_type: str, traj_range: tuple = (-11, -1)
    ):
        trajectories = {}
        trajin_contents = {}
        parmtops = {}
        ...
        (
            ...
            sorted_traj_folders, md_path, traj_range
        )
        ...
        if analysis_type == "diffusion":
            start, end = traj_range
            analysis_folder = f"{analysis_path}/{analysis_type}_{str(start)}_{str(end)}"
        else:
...
def main(args: Args):
    ...
    analysis.traj_analysis(args.md_path, args.analysis_type, args.traj_range)
